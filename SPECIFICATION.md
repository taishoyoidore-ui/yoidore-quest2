...
### 7.1 全酒場 実績集計一覧（オンデマンド消し込み対応）
1. **集計指標（選択中シーズン `season_id` で完全フィルタリング）**:
   - **📷 サイン受取 (来店)**: 選択中シーズンにおいて該当酒場の店頭QRコードがスキャンされたユニーク来店数（`visits WHERE season_id = selectedSeasonId`）。
   - **🍺 クーポン利用 (消込)**: 選択中シーズンにおいて実際に酒場でオンデマンド提示・消し込みが行われた総数（`user_coupons WHERE season_id = selectedSeasonId AND status = 'used' AND reward_type != 'goods'`）。
   - **🎟️ クーポン対象**: 該当酒場がシーズン特典クーポンの利用対象店舗として登録されているか（`season_stores.is_coupon_target`）。
2. **操作・絞り込み機能**:
   - **シーズン連動**: シーズンセレクターを切り替えると、そのシーズンの参加酒場のみを対象に即時再集計。
   - **検索**: 酒場名・酒場IDでのインクリメンタル検索。
   - **エリアフィルター**: 三軒家西、三軒家東、泉尾、平尾などのエリア別絞り込み。
   - **ソート**: サイン受取数順（降順）、クーポン利用数順（降順）、酒場ID順、酒場名順での並び替え。
   - **CSV出力**: 選択中シーズンの全参加酒場実績テーブルデータをExcel互換（UTF-8 BOM付き）CSVとしてワンクリックダウンロード。

### 7.2 特典マイルストーン別 達成・消化集計（一覧表テーブル形式）
カード形式から一覧表（テーブル形式）に刷新され、特典ランクごとの進捗状況を横並びで俯瞰可能。
- **🎁 記念品グッズ引換型**: `達成資格者数`、`受取済点数`、`未受取残数`、`受取進捗率 (%)` を正確に算出・プログレスバーとともに可視化。
- **🍺 酒場クーポン型**: `達成者数`、`発行可能枠総数 (達成者数 × 枠数)`、`店頭消込済数`、`未利用残枠数`、`利用消化率 (%)` を正確に算出・プログレスバーとともに可視化。
...
| バージョン | 改定日 | 主な改定内容 |
| :--- | :--- | :--- |
| **v2026.09.21.23** | 2026-09-21 | **バックオフィス「ハシゴ達成特典（宝箱マイルストーン）設定」および「特典・グッズ別 達成＆消し込み集計」の一覧表（テーブル）形式化**<br>・【特典マイルストーン設定の一覧表形式化】シーズン設定タブ内のハシゴ達成特典設定を従来のカードグリッドからデータテーブル形式に刷新。「必要軒数」「特典種別」「特典名」「特典内容（枠数・グッズ名）」「引換場所/説明」「操作（編集/削除）」の列構成で一元俯瞰・管理可能に改善。<br>・【特典別集計の一覧表形式化】集計分析タブ内の「特典・グッズ別 達成＆消し込み集計」をカード形式からデータテーブル形式に刷新。「必要軒数」「種別」「特典名」「特典内容」「達成者数 (資格者)」「消込・受取済数」「未利用・未受取残数」「消化・受取進捗率」を一覧で比較・可視化。<br>・システム共通バージョンを `v2026.09.21.23` に更新。 |
| **v2026.09.21.19** | 2026-09-21 | **バックオフィス管理画面のデータ集計・表示項目の刷新（オンデマンド消込・ダイレクト受取適合）**<br>・【サマリーKPIの最適化】ダッシュボードおよび集計分析タブにおいて、同値となっていた「獲得数/利用数」を整理し、「📷 総サイン受取 (来店) 数」「🍺 酒場クーポン利用総数 (消込)」「🎁 記念品グッズ受取総数 (点)」「今期参加酒場数」へ実態に即して再編。<br>・【全酒場実績テーブル・CSV出力の刷新】形骸化していた「獲得数」「利用率」列を廃止し、`酒場ID`, `酒場名`, `エリア`, `参加企画`, `📷 サイン受取 (来店)`, `🍺 クーポン利用 (消込)`, `🎟️ クーポン対象` の7列へ集約。ソートセレクターおよびCSV出力も完全連動。<br>・【特典マイルストーン別カード・CSVの精緻化】グッズ引換型（達成人数、受取済数、未受取残数、受取進捗率）と酒場クーポン型（達成人数、発行可能枠総数、利用済数、未利用残枠数、利用消化率）を個別に正確計算・可視化。<br>・【履歴ログ・ユーザー管理の最適化】参加者一覧の「獲得クーポン数」を「特典利用 (消込) 数」に修正。クーポン履歴テーブルに種別（🍺 / 🎁）バッジと利用酒場/記念品名を表示。<br>・システム共通バージョンを `v2026.09.21.19` に更新。 |
| **v2026.09.21.18** | 2026-09-21 | **中世ドラクエ風かすれUSED刻印スタンプ刷新 ＆ メガスタンプ打刻演出 ＆ 履歴リストのシックな中世ログ化 ＆ 勇者称号バナー大型化**<br>・【中世ドラクエ風USEDスタンプ】単調な四角枠から、二重線（実線＋内側点線破線）・かすれグラデーション・ヴィンテージアンティークレッドによる重厚なギルド印章風スタンプ（`.stamp-hanko-badge`, `.treasure-card-stamp-used`）に全面刷新。<br>・【消し込み完了時の特大メガスタンプ】店頭消し込み完了時に、画面奥から3.2倍スケールでドンッと激突する特大メガスタンプ（`.stamp-hanko-mega`、34px、二重破線枠）とスタンプSEの完全同期演出を導入。<br>・【利用済み履歴リストのシックな中世ログ化】ドラクエの瀕死（毒・HP点滅）を連想させていた赤一面のデザインから、落ち着いたダークスレート＆アンティークゴールド（`.tier-usage-item.used`）の高級感ある巡礼記帳ログに刷新。<br>・【冒険の書 勇者称号の大型化 ＆ 重複ラベル削除】LINE名下の称号をゴールドの輝きを持つ大型エンブレム（`.hero-title-badge`、16px太字）に拡大。直下のプログレスバーと重複していた「制覇数」ラベルを撤廃しヘッダーを洗練。<br>・システム共通バージョンを `v2026.09.21.18` に更新。 |
