...
sequenceDiagram
    ...
    Note over User, App: 3. はしご達成 ＆ 宝箱解放（即時枠獲得）
    User->>App: 規定軒数（例: 5軒・10軒・20軒）はしご達成
    App-->>User: 冒険の書の特典カードが「✨ 解放可能」に変化
    alt 酒場クーポン型特典（回数券チケット方式）
        User->>App: 「🎁 宝箱を開ける」をタップ（店舗事前選択なしで即時獲得）
        App->>DB: クーポン枠データを一括保存 (user_coupons: status='active', store_id=null)
        App-->>User: 「🎉 酒場クーポン 5回分 を獲得しました！」（一体型カード内に残回数5/5と利用導線が表示）
    else 記念品・グッズ引換型特典
        User->>App: 「🎁 宝箱を開ける」をタップ ➔ グッズ引換券を即時獲得
        App->>DB: グッズ引換券データを保存 (reward_type: 'goods')
        App-->>User: 「🎁 記念品引換券を獲得しました！」
    end

    Note over User, Shop: 4. 来店時のその都度選択・店頭消し込み
    User->>Shop: 店舗へ来店・お会計時に提示
    alt 冒険の書から利用する場合
        User->>App: 「🏮 お店を選んでクーポンを使う」➔ 対象店舗リストから来店店舗を選択
    else 店舗詳細画面から利用する場合
        User->>App: 店舗詳細画面の「🍺 クーポンを使う」をタップ
    end
    App-->>User: 店頭消し込み確認モーダルを表示
    alt 本開催期間中（クーポン利用期間前）
        App-->>User: 「🔒 クーポン利用期間前（📅 イベント終了翌日〜）」案内表示（消し込みボタン無効化）
    else クーポン利用期間内（後夜祭フェーズ）
        Shop->>App: スタッフ専用「使用済みにする」ボタンをタップ
        App->>DB: 1回分のチケットを特定店舗に割り当て「使用済み (used)」に更新
