...
### 7.2 特典・グッズ別 集計一覧
1. **集計指標**:
   - **特典名・必要来店数・種別**: 店舗クーポン型（`[🍺 店舗クーポン]`）／グッズ引換型（`[🎁 グッズ引換]`）の自動判別表示。
   - **獲得総数**: 当該特典宝箱を開封・獲得した総人数。
   - **引換・消し込み済数**: 店舗または引換所で使用済みにされた総数。
   - **未引換残数**: 現在ユーザーの手元にある未使用引換券数。
   - **進捗バー**: 全体に対する引換消化率を視覚化。
2. **操作・絞り込み機能**:
   - **種別フィルター**: `すべて` / `店舗クーポン型のみ` / `グッズ引換型のみ` の切り替え。
   - **CSV出力**: 特典別の集計データをCSV形式で出力。
