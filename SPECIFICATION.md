...
erDiagram
    ...
    stores {
        ...
        string coupon_description "クーポン特典内容"
        int display_order "並び順"
        jsonb raw_data "店舗詳細統合JSON (hours, days, payment, set_*, quest_*, photo_url, map_url等)"
    }
...
| **ユーザーアプリ** | **酒場ロゴコレクション（図鑑風タイル）** | 冒険の書のハシゴ済み酒場エリアをリストから全店舗のロゴタイルグリッドに刷新。サイン受取前は暗転・グレースケール、サイン受取でカラー＆ゴールド発光点灯。ロゴ画像のみを美しく並べ、タップで店舗名・訪問日時・詳細画面遷移モーダルを表示。 |
| **ユーザーアプリ** | **テスト用サイン記録機能** | 開発・レビュー用として、冒険の書下部に +5店舗、+10店舗、+15店舗のサイン一括付与および履歴リセットボタンを搭載。 |
| **バックオフィス** | **店舗・特典別 集計分析** | 全33店舗の来店・クーポン指名・消し込み実績一覧（ソート・エリア絞り込み対応）、および特典ランク（クーポン型／グッズ型）別の獲得・引換集計とCSV出力。 |
...
| **バックオフィス** | **参加者・データ一覧管理** | 参加者一覧、来店履歴ログ、獲得クーポン利用状況のリアルタイム検索・フィルタリング表示。 |
| **バックオフィス** | **CSVエクスポート機能** | 表示中のタブ（参加者一覧 / 来店サイン履歴 / クーポン獲得・消し込み履歴）の全データをExcel互換（BOM付きUTF-8）CSVとして即時ダウンロード。 |
...
| **バックオフィス** | **アンケートExcel自動取込** | Googleフォーム集計結果のExcel（全38列・セクション分岐）を解析し、最新店舗マスターとしてDBへ一括安全インポート。 |
...
バックオフィス管理画面に新設された「📊 店舗・特典集計」画面（`#analytics`）は、イベント開催中の動向分析や、イベント終了後の各参加店舗への実績フィードバック（送客数・クーポン精算データ）を即座に集計・出力するための機能です。

### 7.1 全店舗 実績集計一覧
1. **集計指標**:
   - **📷 サイン受取数（来店数）**: 該当店舗の店頭QRコードがスキャンされたユニーク件数。
   - **🎟️ クーポン獲得（指名）数**: 参加者がはしご達成特典として該当店舗のクーポンを選択・獲得した総数。
   - **🍺 クーポン利用数（店頭消し込み）**: 実際に店舗で提示され、スタッフにより消し込み操作された総数。
   - **📈 利用率 (%)**: `(利用数 ÷ 獲得数) × 100` で算出。
2. **操作・絞り込み機能**:
   - **検索**: 店舗名・店舗IDでのインクリメンタル検索。
   - **エリアフィルター**: 三軒家西、三軒家東、泉尾、平尾などのエリア別絞り込み。
   - **ソート**: サイン受取数順（降順/昇順）、クーポン獲得数順、利用消し込み数順、利用率順、店舗ID順での並び替え。
   - **CSV出力**: 全店舗の実績テーブルデータをExcel互換（UTF-8 BOM付き）CSVとしてワンクリックダウンロード。

---

## 8. 冒険の書「酒場ロゴコレクション」UI仕様

冒険の書画面の「ハシゴ済み酒場」エリアは、全参加店舗のロゴをタイルのようにグリッド配置した**図鑑風コレクションUI**として設計されています。

### 8.1 タイル表示ルール
1. **全店グリッド配置**:
   - 参加全店舗（`store-01` 〜 `store-33`）を連番順に整列。
   - スマートフォン幅で4列（幅広画面で5列）の正方形タイル（アスペクト比 1:1）で一覧表示。
2. **未制覇（サイン未受取）**:
   - グレースケール（白黒）＋ 透過度35% ＋ 暗転背景（`filter: grayscale(100%) brightness(35%) contrast(120%); opacity: 0.35;`）。
   - まだ訪れていない場所がシルエットのように浮かび上がり、探索意欲を向上。
3. **制覇済み（サイン受取後）**:
...
| バージョン | 改定日 | 主な改定内容 |
| :--- | :--- | :--- |
| **v2026.09.18.25** | 2026-09-18 | **冒険の書「酒場ロゴコレクション（図鑑風タイル）」刷新**<br>・ハシゴ済み酒場エリアを従来のテキストリストから全店ロゴタイルのグリッドUIに刷新。<br>・未制覇は暗転・反転、制覇でカラー＆ゴールド発光点灯。<br>・タイルタップで店舗名・訪問日時確認および酒場詳細画面への遷移モーダルを実装。<br>・システム共通バージョンを `v2026.09.18.25` に更新。 |
